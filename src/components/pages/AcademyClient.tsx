...
export default function AcademyClient({
  ...
  const [academy, setAcademy] = useState<Academy | null>(initialAcademy);
  const [players, setPlayers] = useState<Player[]>(initialPlayers);

  /* The server already fetched the academy and its squad, so the squad names are
     in the initial HTML. Only fall back to a browser fetch if it got nothing. */
  const serverHadNothing = !initialAcademy && initialPlayers.length === 0;
          ...
          <div className="relative aspect-[4/3] bg-surface rounded-2xl border border-line/60 overflow-hidden animate-academy-photos">
            {academy?.photo ? (
              <Image
                ...
                sizes="(max-width: 640px) 100vw, (max-width: 768px) 75vw, 66vw"
              />
            ) : (
