...
import { deriveTeamState, SQUAD_PLAYERS_PAGE_SIZE, type DerivedTeamState } from "@/lib/squad";
...
/* Payload the server fetched (see src/app/(public)/squad/page.tsx). Serializable
   only — Sets become arrays so they can cross the server → client boundary. */
export type SquadInitialData = {
  playersPage: { data: Player[]; totalPages: number };
  statistics: Statistic[];
  teams: Team[];
  scheduledMatches: Match[];
  liveMatches: Match[];
  teamState: DerivedTeamState;
  nextMatch: Match | null;
};

export default function SquadClient({ initialData }: { initialData: SquadInitialData }) {
  ...
  const serverHadNothing =
    initialData.playersPage.data.length === 0 && initialData.teams.length === 0;

  const [players, setPlayers] = useState<Player[]>(initialData.playersPage.data);
  ...
  const [statistics, setStatistics] = useState<Statistic[]>(initialData.statistics);
  ...
  const [teamFormationName, setTeamFormationName] = useState<string>(initialData.teamState.teamFormationName ?? "4-3-3");

  // Team / captain / startingXI state — seeded from the server render so the
  // starting XI and bench appear in the initial HTML.
  const [captainId, setCaptainId] = useState<string | null>(initialData.teamState.captainId);
  const [viceCaptainId, setViceCaptainId] = useState<string | null>(initialData.teamState.viceCaptainId);
  const [teamStartingXI, setTeamStartingXI] = useState<StartingXIEntry[]>(initialData.teamState.teamStartingXI);
  const [teamPlayerIds, setTeamPlayerIds] = useState<Set<string>>(new Set(initialData.teamState.teamPlayerIds));
  const [teamBench, setTeamBench] = useState<Player[]>(initialData.teamState.teamBench);

  // Next match state
  const [nextMatch, setNextMatch] = useState<Match | null>(initialData.nextMatch);
  const [loadingMatch, setLoadingMatch] = useState(serverHadNothing);  // Match formation state (from MatchFormation API)
  ...
  const [hasMore, setHasMore] = useState(initialData.playersPage.totalPages > 1);
  const pageRef = useRef(1);
  const hasMoreRef = useRef(initialData.playersPage.totalPages > 1);
  ...
  useEffect(() => {
    fetchData(serverHadNothing ? undefined : initialData);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const fetchData = async (seed?: SquadInitialData) => {
    // When the server already fetched everything we skip the network entirely but
    // still run the same processing below, so the derived state (captain,
    // formation, starting XI, next match) ends up identical.
    if (!seed) setLoading(true);
    try {
      let playersRes: any, statsRes: any, teamRes: any, matchRes: any, liveMatchRes: any;
      if (seed) {
        // Shape the seed like the axios responses the code below expects.
        playersRes = { status: "fulfilled", value: { data: seed.playersPage } };
        statsRes = { status: "fulfilled", value: { data: { data: seed.statistics } } };
        teamRes = { status: "fulfilled", value: { data: { data: seed.teams } } };
        matchRes = { status: "fulfilled", value: { data: { data: seed.scheduledMatches } } };
        liveMatchRes = { status: "fulfilled", value: { data: { data: seed.liveMatches } } };
      } else {
        [playersRes, statsRes, teamRes, matchRes, liveMatchRes] = await Promise.allSettled([
          api.get("/players", { params: { limit: SQUAD_PLAYERS_PAGE_SIZE, page: 1, sort: "lastName" } }),
      ...
      // Extract captain, formation and starting XI from team data — prefer SENIOR team
      let firstTeamId: string | null = null;
      if (teamRes.status === "fulfilled") {
        const allTeams: Team[] = teamRes.value.data.data || [];
        const hasTeam = Array.isArray(allTeams) && allTeams.length > 0;
        const derived = deriveTeamState(allTeams);
        firstTeamId = derived.firstTeamId;
        if (hasTeam) {
          if (derived.captainId) setCaptainId(derived.captainId);
          if (derived.viceCaptainId) setViceCaptainId(derived.viceCaptainId);
          if (derived.teamFormationName) setTeamFormationName(derived.teamFormationName);
          if (derived.teamStartingXI.length > 0) setTeamStartingXI(derived.teamStartingXI);
          if (derived.teamBench.length > 0) setTeamBench(derived.teamBench);
          setTeamPlayerIds(new Set(derived.teamPlayerIds));
        }
      }
  ...
  const loadMorePlayers = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/players", {
        params: { limit: SQUAD_PLAYERS_PAGE_SIZE, page: nextPage, sort: "lastName" },
      });
        ...
        <div className="flex gap-1 mb-10 overflow-x-auto pb-2">
          {positionFilters.map((pos) => (
            <button
              ...
              aria-label={`Filter by ${pos.label}`}
            >
              {pos.label}
            </button>
          ))}
              ...
              <div className="flex items-center justify-center gap-2 flex-wrap">
                {FORMATION_OPTIONS.map((name) => (
                  <button
                    ...
                    aria-label={`Select ${name} formation`}
                  >
                    {name}
                  </button>
                ))}
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-line/80 bg-surface-raised shadow-sm transition-all duration-200 group-hover:border-pitch-accent group-hover:shadow-md group-hover:scale-105 relative">
          {player.photo ? (
            <Image
              src={player.photo}
