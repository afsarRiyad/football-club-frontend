...
export default function StandingsClient({
  ...
  /* Position is derived from array order, so apply it to the server data too. */
  const [standings, setStandings] = useState<Standing[]>(() =>
    initialStandings.map((s, i) => ({ ...s, position: i + 1 })),
  );
  const [topScorers, setTopScorers] = useState<TopScorer[]>(initialTopScorers);

  /* The server already rendered the table, so skip the first fetch unless it
     came back empty (the API was unreachable). */
  const serverHadNothing =
    initialStandings.length === 0 && initialTopScorers.length === 0;
