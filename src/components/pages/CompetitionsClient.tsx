                  ...
                  >
                    <div className={cn("flex items-center gap-1.5 px-2.5 py-1.5 border-b border-line/30", isHomeWin && "bg-green-500/5")}>
                      {getTeamLogo(match.homeTeam) && (
                        <Image src={getTeamLogo(match.homeTeam)} alt={`${getTeamName(match.homeTeam)} logo`} width={16} height={16} className="w-4 h-4 rounded object-contain" />
                      )}
                    ...
                    <div className={cn("flex items-center gap-1.5 px-2.5 py-1.5", isAwayWin && "bg-green-500/5")}>
                      {getTeamLogo(match.awayTeam) && (
                        <Image src={getTeamLogo(match.awayTeam)} alt={`${getTeamName(match.awayTeam)} logo`} width={16} height={16} className="w-4 h-4 rounded object-contain" />
                      )}
  ...
  return (
    ...
    >
      ...
      <div className="relative h-32 bg-gradient-to-br from-surface-raised to-surface overflow-hidden">
        {competition.logo ? (
          <Image src={competition.logo} alt={competition.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, 33vw" />
        ) : (
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-3 mt-3 text-xs text-mist">
          ...
          {competition.format && (
            <span className="text-mist/70">{competition.format.replace(/_/g, " ")}</span>
          )}
        </div>
      </div>
    </button>
...
/* Competitions and tournaments come from two endpoints but share one list in the
   UI. Tournaments are tagged so the detail view knows which API to call. */
type TournamentEntry = Competition & { _source?: "tournament" };
...
function mergeCompetitions(
  ...
  return [
    ...
    ...tournaments.map((t) => ({
      ...t,
      type: "TOURNAMENT" as Competition["type"],
      _source: "tournament" as const,
    })),
  ];
}

export type CompetitionsInitialData = {
  competitions: Competition[];
  tournaments: Competition[];
};
...
export default function CompetitionsClient({
  ...
  initialData: CompetitionsInitialData;
}) {
  const [competitions, setCompetitions] = useState<Competition[]>(() =>
    mergeCompetitions(initialData.competitions, initialData.tournaments),
  );

  /* The server already rendered the list, so skip the first fetch unless it
     came back empty (the API was unreachable). */
  const serverHadNothing =
    initialData.competitions.length === 0 && initialData.tournaments.length === 0;
  ...
  const fetchCompetitions = async () => {
    ...
    try {
      ...
      setCompetitions(mergeCompetitions(comps, tourns));
    } catch (e) {
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="relative h-48 md:h-64 rounded-2xl overflow-hidden mb-8">
          {selected.logo ? (
            <Image src={selected.logo} alt={selected.name} fill className="object-cover" sizes="100vw" />
          ) : (
                ...
                return (
                  <div key={label} className="bg-surface rounded-xl border border-line/40 overflow-hidden">
                              ...
                              <tr key={i} className={cn("border-b border-line/20 last:border-0", i < 2 && "bg-emerald-500/5")}>
                                ...
                                <td className="py-2 px-3">
                                  <div className="flex items-center gap-2">
                                    {tl ? <Image src={tl} alt={`${tn} logo`} width={20} height={20} className="w-5 h-5 rounded-full object-contain" /> : <div className="w-5 h-5 rounded-full bg-surface-raised flex items-center justify-center text-[7px] font-bold text-line">{tn?.charAt(0)}</div>}
                    ...
                    <div className="px-4 py-1.5 border-t border-line/20">
                      <p className="text-[9px] text-mist/70">Top 2 advance to knockout</p>
                    </div>
                  </div>
                );
              })}
                        ...
                        <tr key={i} className="border-b border-line/20 last:border-0 hover:bg-surface-raised/50 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              {teamLogo ? (
                                <Image src={teamLogo} alt={`${teamName} logo`} width={28} height={28} className="w-7 h-7 rounded-full object-contain border border-line/30" />
                              ) : (
                          ...
                          >
                            ...
                            <div className={cn(
                              ...
                              {isLive ? (
                                ...
                                <span className="text-[10px] font-mono text-mist/70">UPCOMING</span>
                              ) : null}
                            </div>
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 text-right">
                                  <div className="flex items-center justify-end gap-3">
                                    ...
                                    {homeLogo ? (
                                      <Image src={homeLogo} alt={`${homeName} logo`} width={40} height={40} className="w-10 h-10 rounded-full object-contain border border-line/30" />
                                    ) : (
                                ...
                                <div className="flex flex-col items-center shrink-0">
                                  {isCompleted ? (
                                    <div className="flex items-center gap-2">
                                      <span className={cn("text-2xl font-black font-mono", isHomeWin ? "text-floodlight" : "text-mist/70")}>{m.homeScore ?? 0}</span>
                                      <span className="text-lg text-line/30 font-light">-</span>
                                      <span className={cn("text-2xl font-black font-mono", isAwayWin ? "text-floodlight" : "text-mist/70")}>{m.awayScore ?? 0}</span>
                                    </div>
                                  ) : (
                                ...
                                <div className="flex-1 text-left">
                                  <div className="flex items-center gap-3">
                                    {awayLogo ? (
                                      <Image src={awayLogo} alt={`${awayName} logo`} width={40} height={40} className="w-10 h-10 rounded-full object-contain border border-line/30" />
                                    ) : (
                                      <div className="w-10 h-10 rounded-full bg-surface-raised border border-line/30 flex items-center justify-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {(selected as any).teams.filter((t: any) => t && typeof t === "object").map((team: any, i: number) => (
                <div key={i} className="flex items-center gap-3 bg-surface rounded-xl border border-line/40 px-4 py-3">
                  {team.logo ? (
                    <Image src={team.logo} alt={`${team.name} logo`} width={32} height={32} className="w-8 h-8 rounded-full object-cover border border-line" />
                  ) : (
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <p className="text-sm text-mist/70 mt-1">Check back later for updates.</p>
        </div>
      ) : (
